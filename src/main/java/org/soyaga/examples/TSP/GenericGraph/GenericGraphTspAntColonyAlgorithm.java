 ...
 */
public class GenericGraphTspAntColonyAlgorithm implements AntColonyAlgorithm {
     ...
     */
    HashMap<Integer, RunGenericGraphTspOptimization.tspNode> worldMap;
    /**
     ...
     */
    public GenericGraphTspAntColonyAlgorithm(String ID, World world, Colony colony, Integer initialColonySize, Ant ant,
        ...
        this.radius = scale/2.;
        this.imageWidth = (int) (this.width * this.scale + 2 * this.radius);
        this.imageHeight = (int) (this.height * this.scale + 2 * this.radius);
        creteInitialImages();
    ...
    public Object getResult() {
        ...
        graphics2D.setStroke(new BasicStroke(this.imageHeight/200));
        Integer node1ID= (Integer) ((Node) solution.getStartNode()).getID();
        for(Object edge: solution.getPath()) {
            Integer node2ID= (Integer) ((Node)this.world.getGraph().getNextNode((Edge)edge)).getID();
            RunGenericGraphTspOptimization.tspNode node1 = this.worldMap.get(Integer.valueOf(node1ID));
            RunGenericGraphTspOptimization.tspNode node2 = this.worldMap.get(Integer.valueOf(node2ID));
            graphics2D.drawLine(
    ...
    private void creteInitialImages() {
        ...
        AffineTransform at = AffineTransform.getScaleInstance(1, -1);
        graphics2D.setTransform(at);
        for(RunGenericGraphTspOptimization.tspNode node: this.worldMap.values()){
        ...
        graphics2D.setTransform(at);
        Stroke dashed = new BasicStroke(this.imageHeight/1000, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL,
                0, new float[]{this.imageHeight/100}, 0);
        graphics2D.setStroke(dashed);
        for(RunGenericGraphTspOptimization.tspNode node1: this.worldMap.values()){
            for (Integer node2Id:node1.getRelatedNodes()){
                RunGenericGraphTspOptimization.tspNode node2 = this.worldMap.get(node2Id);
                graphics2D.drawLine(
                        (int) ((node1.getX()-this.minX)*scale+this.radius),
                        (int) ((node1.getY()-this.minY)*scale-imageHeight+this.radius),
                        (int) ((node2.getX()-this.minX)*scale+this.radius),
