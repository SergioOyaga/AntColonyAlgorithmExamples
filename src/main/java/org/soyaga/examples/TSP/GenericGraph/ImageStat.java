...
import org.soyaga.examples.TSP.GenericGraph.RunGenericGraphTspOptimization.tspNode;
...
public class ImageStat implements Stat {
    ...
    private Image getImage(World world, Colony colony, Integer generation){
        ...
        Integer node1ID= (Integer) ((Node)solution.getNodesVisited().get(0)).getID();
        for(Object edge: solution.getEdgesUtilized()) {
            Integer node2ID= (Integer) ((Node)world.getGraph().getNextNode(edge)).getID();
        ...
        tspNode node = this.worldMap.get((Integer) ((Node)solution.getNodesVisited().get(0)).getID());
        ...
        node = this.worldMap.get((Integer) ((Node)solution.getNodesVisited().get(solution.getNodesVisited().size()-1)).getID());
    ...
    private void creteInitialImages() {
        ...
        graphics2D.setTransform(at);
        Stroke dashed = new BasicStroke((float) 1, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL,
                0, new float[]{(float) 10}, 0);
