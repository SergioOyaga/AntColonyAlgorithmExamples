 ...
 */
public class DenseTspAntColonyAlgorithm implements AntColonyAlgorithm {
     ...
     */
    HashMap<Integer, RunDenseTspOptimization.tspNode> worldMap;
    /**
     * HashSet with the nodes and coordinates.
     */
    HashMap<Integer, RunDenseTspOptimization.tspNode> worldMapOut;
    /**
     ...
     */
    public DenseTspAntColonyAlgorithm(String ID, World world, Colony colony, Integer initialColonySize, Ant ant,
                                      ...
                                      HashMap<Integer, RunDenseTspOptimization.tspNode> worldMap,
                                      HashMap<Integer, RunDenseTspOptimization.tspNode> worldMapOut,Integer scale) {
        super();
        this.ID = ID;
        this.world = world;
        this.colony = colony;
    ...
    public Object getResult() {
        ...
        graphics2D.setStroke(new BasicStroke(this.imageHeight/100));
        ...
        for(Object edge: solution.getPath()) {
            ...
            RunDenseTspOptimization.tspNode node1 = this.worldMapOut.get(node1ID);
            RunDenseTspOptimization.tspNode node2 = this.worldMapOut.get(node2ID);
            graphics2D.drawLine(
    ...
    private void creteInitialImages() {
        ...
        AffineTransform at = AffineTransform.getScaleInstance(1, -1);
        graphics2D.setTransform(at);
        for(RunDenseTspOptimization.tspNode node: this.worldMap.values()){
        ...
        for(RunDenseTspOptimization.tspNode node1: this.worldMap.values()){
            for (Integer node2Id:node1.getRelatedNodes()){
                RunDenseTspOptimization.tspNode node2 = this.worldMap.get(node2Id);
                graphics2D.drawLine(
                        (int) ((node1.getX()-this.minX)*scale+this.radius),
                        (int) ((node1.getY()-this.minY)*scale-imageHeight+this.radius),
                        (int) ((node2.getX()-this.minX)*scale+this.radius),
