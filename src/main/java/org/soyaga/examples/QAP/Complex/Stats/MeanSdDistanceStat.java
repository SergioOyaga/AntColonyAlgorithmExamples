...
/**
 * Stat that computes the mean and standard deviation of the distance for the colony.
 */
public class MeanSdDistanceStat implements Stat {
    /**
     ...
     */
    public MeanSdDistanceStat(Integer decimalNumber, ArrayList<Facility> facilities) {
        this.header = new ArrayList<>(){{add("MeanDist");add("StrDevDist");}};
     ...
     * colony mean distance and standard deviation values.
     *
    ...
    public ArrayList<String> apply(World world, Colony colony, Object... statArgs) {
        ...
        Double mean = distances.stream().reduce(0., Double::sum)/distances.size();
        double sd = distances.stream().
